real(8) function db_cov_M_expected_antineutrino_number_far_bin(j,k,t13,dmee)    
    implicit none
    integer :: j,k
    real(8) :: t13, dmee    
    real(8) :: db_cov_M_expected_antineutrino_number_hall_period_bin  ! (H,P,j,j,k,t13,dmee)
    real(8) :: result
    integer :: H,P    
    result=0.0d0
    if(j<=26) then
    ...
    db_cov_M_expected_antineutrino_number_far_bin=result
    return
end function db_cov_M_expected_antineutrino_number_far_bin