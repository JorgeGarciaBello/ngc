function reno_partial_derivative_energy_scale(b_d,fr,eps,eta)
    use  types
    use reno_data, only: NBIN,ADS,RCTS,nearObs,farObs,bkg,sigma_energy_scale,data
    implicit none
    real(dp) :: reno_partial_derivative_energy_scale
    ...
    reno_partial_derivative_energy_scale= eta/(sigma_energy_scale/100.0d0)**2
    result=0.0_dp
    do i=1,NBIN
      N_near=0.0d0;
      N_far=0.0d0;
      ...
      result=result + ((T_i - O_i)/U2_i)*(N_far*nearExpC - farExpC*N_near)/(nearExpC)**2
    enddo
    reno_partial_derivative_energy_scale=2.0_dp*(reno_partial_derivative_energy_scale + result)
end function reno_partial_derivative_energy_scale