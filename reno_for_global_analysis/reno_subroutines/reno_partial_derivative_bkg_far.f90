function reno_partial_derivative_bkg_far(b_d,fr,eps,eta)
    use  types
    use reno_data, only: NBIN,ADS,RCTS,nearObs,farObs,bkg,sigma_background_d, &
                         data
    implicit none
    real(dp) :: reno_partial_derivative_bkg_far
    ...
    reno_partial_derivative_bkg_far= (b_d(2))/(sigma_background_d(2)/100.0d0)**2
    result=0.0_dp
    do i=1,NBIN
      N_near=0.0d0;
      N_far=0.0d0;
      ...
      result=result + ((T_i - O_i)/U2_i)*( bkg(i,2)/nearExpC )
    enddo
    reno_partial_derivative_bkg_far=2.0_dp*(reno_partial_derivative_bkg_far+result)
end function reno_partial_derivative_bkg_far