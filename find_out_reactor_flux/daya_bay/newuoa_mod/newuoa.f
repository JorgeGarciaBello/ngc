      SUBROUTINE NEWUOA_M (N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,W,
     1  XX,YX)
...
C
      ...
      IF (NPT .LT. N+2 .OR. NPT .GT. ((N+2)*NP)/2) THEN
          PRINT 10
   10     FORMAT (/4X,'Return from NEWUOA_M because NPT is not in',
     1      ' the required interval')
      ...
      IW=IVL+NDIM
C
C     The above settings provide a partition of W for subroutine NEWUOB_M.
C     The partition requires the first NPT*(NPT+N)+5*N*(N+3)/2 elements of
C     W plus the space that is needed by the last array of NEWUOB_M.
C
      CALL NEWUOB_M (N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,W(IXB),
     1  W(IXO),W(IXN),W(IXP),W(IFV),W(IGQ),W(IHQ),W(IPQ),W(IBMAT),
     2  W(IZMAT),NDIM,W(ID),W(IVL),W(IW),XX,YX)
   20 RETURN
      END
