      SUBROUTINE NEWUOB_M (N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,XBASE,
     1  XOPT,XNEW,XPT,FVAL,GQ,HQ,PQ,BMAT,ZMAT,NDIM,D,VLAG,W,XX,YX)
...
C
  100 KNEW=0
      CALL TRSAPP_M (N,NPT,XOPT,XPT,GQ,HQ,PQ,DELTA,D,W,W(NP),
...
C     may be made later, if the choice of D by BIGLAG_M causes substantial
C     cancellation in DENOM.
C
      IF (KNEW .GT. 0) THEN
          CALL BIGLAG_M (N,NPT,XOPT,XPT,BMAT,ZMAT,IDZ,NDIM,KNEW,DSTEP,
...
C     then BIGDEN_M calculates an alternative model step, XNEW being used for
C     working space.
C
      IF (KNEW .GT. 0) THEN
          TEMP=ONE+ALPHA*BETA/VLAG(KNEW)**2
          IF (DABS(TEMP) .LE. 0.8D0) THEN
              CALL BIGDEN_M (N,NPT,XOPT,XPT,BMAT,ZMAT,IDZ,NDIM,KOPT,
...
C
  ...
  320     FORMAT (/4X,'Return from NEWUOA because CALFUN_M has been',
     1      ' called MAXFUN times.')
          GOTO 530
      END IF
      CALL CALFUN_M (N,X,F,XX)
      IF (IPRINT .EQ. 3) THEN
...
C
C     Update_M FOPT and XOPT if the new F is the least value of the objective
...
C
C     Update_M BMAT, ZMAT and IDZ, so that the KNEW-th interpolation point
C     can be moved. Begin the updating of the quadratic model, starting
C     with the explicit second derivative term.
C
  410 CALL UPDATE_M (N,NPT,BMAT,ZMAT,IDZ,NDIM,VLAG,BETA,KNEW,W)
      FVAL(KNEW)=F
      IH=0
      DO 420 I=1,N
  ...
  420 HQ(IH)=HQ(IH)+TEMP*XPT(KNEW,J)
      PQ(KNEW)=ZERO
C
C     Update_M the other second derivative parameters, and then the gradient
...
c              PRINT 520, FOPT,(XBASE(I)+XOPT(I),I=1,N)
  520         FORMAT (4X,'Lesast value of F =',1PD23.15,9X,
     1          'The corresponding X is:'/(2X,5D15.6))
          END IF
          GOTO 90
      END IF
