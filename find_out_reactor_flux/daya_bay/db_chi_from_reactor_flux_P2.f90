subroutine db_chi_from_reactor_flux_P2(t13,dmee,X,chi)
    use types
    use db_data, only: NDIM, NBIN, N_obs_H3_8AD
    implicit none
    ...
    do bin=1,NBIN
        chi = chi + &
            (N_obs_H3_8AD(bin) - db_expected_far_events_from_near_H1_data_P2(bin,t13,dmee,X))**2/(N_obs_H3_8AD(bin)*alp)
    enddo
    do bin=1,NBIN
        chi = chi + &
