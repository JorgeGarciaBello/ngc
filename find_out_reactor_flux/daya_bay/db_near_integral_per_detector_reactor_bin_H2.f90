function db_near_integral_per_detector_reactor_bin_H2(d,r,bin,t13,dmee)
    use types
    ...
    if ((d==3).or.(d==4)) then 
        ...
        enddo
        db_near_integral_per_detector_reactor_bin_H2 = result/( length_d_r(d,r)**2 )
    else
       print*, 'El detector para la sala H2 no coincide con los indices: 3,4' 
       stop
    end if
    return
end function db_near_integral_per_detector_reactor_bin_H2