function db_expected_far_events_from_near_H1_data_P3(bin,t13,dmee,x)
    use types
    use db_data, only: NDIM, NBIN, RCTS, TP_d, LT_d, N_obs_H1_7AD, Bkg_H1_7AD, Bkg_H3_7AD
    implicit none
    integer, parameter :: PRD=3   ! Period of data taking
    real(dp) :: db_expected_far_events_from_near_H1_data_P3
    integer  :: bin
    real(dp) :: t13,dmee,x(NDIM)
    real(dp) :: db_near_integral_per_detector_reactor_bin_H1_P3
    ...
    enddo
    r3 = r3*(N_obs_H1_7AD(bin) - Bkg_H1_7AD(bin)*x(1))    
    r1 = 0.0_dp
    do r=1,RCTS
        r1 = r1 + TP_d(1)*LT_d(PRD,1)*db_near_integral_per_detector_reactor_bin_H1_P3(1,r,bin,t13,dmee,X) + &
                  TP_d(2)*LT_d(PRD,2)*db_near_integral_per_detector_reactor_bin_H1_P3(2,r,bin,t13,dmee,X)
    enddo    
    db_expected_far_events_from_near_H1_data_P3 = 0.83*(1.0_dp+x(4))*(r3/r1) - Bkg_H3_7AD(bin)*x(3)
    return
