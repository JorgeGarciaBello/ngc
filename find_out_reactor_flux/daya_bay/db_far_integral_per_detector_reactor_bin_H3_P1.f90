function db_far_integral_per_detector_reactor_bin_H3_P1(d,r,bin,t13,dmee,xy)
    use types
    ...
    if((d>=5).or.(d<=8)) then
        ...
        enddo
        db_far_integral_per_detector_reactor_bin_H3_P1 = result/( length_d_r(d,r)**2 )
    else
        print*, 'El indice del detector para la sala H3 no se encuentra dentro del rango 5 <= d <= 8'
    end if
    return
end function db_far_integral_per_detector_reactor_bin_H3_P1