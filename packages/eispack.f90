...
!
      if ( abs ( h(na,en) ) < abs ( h(en,na) ) ) then
...
!
          ...
          call cdiv ( x * r - zz * ra + q * sa, x * s-zz * sa - q * ra, &
            vr, vi, h(i,na), h(i,en) )

          if ( abs ( zz ) + abs ( q ) < abs ( x ) ) then
            h(i+1,na) = ( - ra - w * h(i,na) + q * h(i,en) ) / x
            h(i+1,en) = ( - sa - w * h(i,en) - q * h(i,na) ) / x
          else
            call cdiv ( - r - y * h(i,na), - s - y * h(i,en), zz, q, &
              h(i+1,na), h(i+1,en) )
          end if
